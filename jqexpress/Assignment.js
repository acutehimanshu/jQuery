...
app.get('/employees', async function(req, res){
 ...
 const makeConnection = async (methodCalled)=>{
...
app.get('/departments', async function(request, response){
    await (makeConnection(async (connection)=>{
        const reesultset = await connection.execute("SELECT * FROM departments"); 
        response.send(reesultset.rows);
    }));
});

app.get('/employeeBydepartment', async function(request, response){
    var id = request.query.id;
    console.log('employeeBydepartment');
    await (makeConnection(async (connection)=>{
        const reesultset = await connection.execute("SELECT employee_id, first_name, department_id FROM employees where department_id = "+id); 
        response.send(reesultset.rows);
    }));
});
